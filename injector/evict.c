			      ...
			      unsigned long nr_pages, bool *skip)
{
	...
	for_each_zone_zonelist_nodemask (zone, z, zonelist, MAX_NR_ZONES - 1,
					 NULL) {
		...
		for_each_evictable_anon_lru(lru)
		{
			...
			memset(evict.print_buf, 0, sizeof(evict.print_buf));
			...
			printk(KERN_INFO "lru: %d %s %d:%s %ld %s", nid,
			       ...
			       lru == LRU_INACTIVE_ANON ? "inactive" : "active",
